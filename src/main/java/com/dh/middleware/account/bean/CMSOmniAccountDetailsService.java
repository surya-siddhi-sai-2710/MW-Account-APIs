...
public class CMSOmniAccountDetailsService {
	  ...
	  public void prepareXMLResponse(Exchange exchange)
		  ...
		  if (returncode.equals("0000")) {
			  ...
			  oCmsOmniAccountDetailsResponseType.setStatusCode(oAccountUtils.getValueFromCMSResponse("RETURNCODE", responseString));
			  oCmsOmniAccountDetailsResponseType.setStatusDescription(oAccountUtils.getValueFromCMSResponse("DESCRIPTION", responseString));
			  ...
			  if (root != null) {
				  JsonNode accdataArrays = root.get("OMNIDATA");
			        ...
			        for (JsonNode accdata : accdataArrays) {
			        	...
			        	oOmniAccountType.setOmniMappedCode(oStringUtil.setDefaultValue(accdata.get("OMNIMAPPEDCODE"), null));
			        	oOmniAccountType.setOmniAccountType(oStringUtil.setDefaultValue(accdata.get("OMNIACCOUNTTYPE"), null));
			        	...
			        	oOmniAccountType.setMainStatusDesciption(oStringUtil.setDefaultValue(accdata.get("MSTATUSDESC"), null));
			        	...
			        	oOmniAccountType.setAccountOwnerType(oStringUtil.setDefaultValue(accdata.get("ACCOWNERTYPE"), null));
			        	oOmniAccountType.setAccountOwnerTypeDescription(oStringUtil.setDefaultValue(accdata.get("ACCOWNERTYPEDESC"), null));
			        	oOmniAccountType.setLastSequenceOfAccount(oStringUtil.setDefaultValue(accdata.get("ACCOUNTSEQNB"), null));
			        	oOmniAccountType.setBranchCode(oStringUtil.setDefaultValue(accdata.get("BRNCD"), null));
			        	oOmniAccountType.setCurrencyNumericId(oStringUtil.setDefaultValue(accdata.get("CURRENCYNUMERICID"), null));
			        	oOmniAccountType.setCifNumber(oStringUtil.setDefaultValue("string", null));
			        	oOmniAccountType.setTransactionNotificationFlag(oStringUtil.setDefaultValue(accdata.get("DISNOTIFICATIONENABLED"), null));
			        	oOmniAccountType.setAllowedTransactions(oStringUtil.setDefaultValue(accdata.get("DALLOWEDTRNTYPES"), null));
			        	...
			        	oOmniAccountType.setNumberOfVirtualAccounts(oStringUtil.setDefaultValue(accdata.get("NUMBEROFVIRTUALACCOUNTS"), null));
			        	oOmniAccountType.setLastUpdateTime(oStringUtil.setDefaultValue(accdata.get("LASTUPDATETIME"), null));
			        	 ...
			        	 List<AllowedTransactionFormatedType> oAllowedTransactionFormatedTypes = new ArrayList<>();
			                ...
			                if (transactionsNode != null) {
			                    for (JsonNode transactionNode : transactionsNode) {
			                        ...
			                        oOmniTransactions.setOrderBy(oStringUtil.setDefaultValue(transactionNode.get("ORDERBY"), null));
			                        oOmniTransactions.setTransactionCode(oStringUtil.setDefaultValue(transactionNode.get("TRANSACTIONCD"), null));
			                        oOmniTransactions.setTransactionCodeDescription(oStringUtil.setDefaultValue(transactionNode.get("TRANSACTIONNAME"), null));
			                        oOmniTransactions.setIsSelected(oStringUtil.setDefaultValue(transactionNode.get("ISSELECTED"), null));
